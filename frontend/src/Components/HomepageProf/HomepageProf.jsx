...
function HomepageProf({ onLoginSuccess }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        axios
          .get(
            `http://localhost:8080/api/registration-session/${response.data.userId}`,
            {
              headers: {
  ...
  const createSession = () => {
    const startTimeUnix =
      Math.floor(new Date(sessionValues[1]).getTime() / 1000) * 1000;
    const endTimeUnix =
      Math.floor(new Date(sessionValues[2]).getTime() / 1000) * 1000;
    axios
      .post(
        `${url}registration-session/create`,
        {
